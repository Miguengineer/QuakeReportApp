    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            for (int i = 0; i < jsonFeaturesArray.length(); i++){
                JSONObject JSONObject = jsonFeaturesArray.getJSONObject(i).
                        getJSONObject("properties");
                mag = JSONObject.optDouble("mag");
                date = JSONObject.optLong("time");
                location = JSONObject.optString("place");
                earthquakes.add(new Earthquake(mag, location, new Date(date)));
            }
